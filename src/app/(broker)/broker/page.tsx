...
export default function BrokerDashboard() {
  ...
  const loadDashboard = async () => {
    ...
    const { data: profile } = await supabase.from('profiles').select('name, wallet_balance').eq('id', user.id).single()
    const { data: props } = await supabase.from('properties').select('id, title, area, price, status, created_at, rejection_reason, description, address, images, unit_type').eq('owner_id', user.id).order('created_at', { ascending: false })

    const propertyIds = props?.map(p => p.id) ?? []
    let leadsCount = 0
    if (propertyIds.length > 0) {
      const { count } = await supabase.from('leads').select('*', { count: 'exact', head: true }).in('property_id', propertyIds)
      leadsCount = count ?? 0
    }

    setStats({ name: profile?.name ?? '', walletBalance: profile?.wallet_balance ?? 0, totalProperties: props?.length ?? 0, totalLeads: leadsCount })
    setProperties(props ?? [])
    setLoading(false)
  }
