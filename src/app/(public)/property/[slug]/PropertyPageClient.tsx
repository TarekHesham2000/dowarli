...
export default function PropertyPageClient() {
  ...
  useEffect(() => {
    const load = async () => {
      const segment = decodePathSegment(slugParam)
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box;}
        body { background: #f9fafb; font-family: var(--font-cairo), 'Cairo', sans-serif; }
        @keyframes shimmer {
      ...
      `}</style>
      <div dir="rtl" style={{ minHeight: '100vh', background: '#f9fafb', color: '#0f172a' }}>
        <nav style={{ position: 'sticky', top: 0, zIndex: 100, background: '#ffffff', borderBottom: '1px solid #e5e7eb', padding: '0 1.25rem', height: 48, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2.5rem 1.5rem 6rem' }}>
          ...
          <div className="sk-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '2rem', alignItems: 'start' }}>
            <div>
              ...
              <div style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 20, padding: '1.5rem', marginBottom: '1.5rem' }}>
              ...
              <div style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 20, padding: '1.5rem' }}>
            ...
            <div>
              <div style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '1.6rem', marginBottom: '1rem' }}>
              ...
              <div style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '1.6rem' }}>
  ...
  // ══════════════════════════════════════════════════════════
  if (!property) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f9fafb', fontFamily: "var(--font-cairo), 'Cairo', sans-serif", direction: 'rtl' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 72, marginBottom: 16 }}>🏚️</div>
        <p style={{ color: '#64748b', fontSize: 18, fontWeight: 700, marginBottom: 8 }}>الإعلان غير موجود</p>
        <p style={{ color: '#475569', fontSize: 13, marginBottom: 24 }}>ربما تم حذف الإعلان أو الرابط غير صحيح</p>
        <button onClick={() => router.push('/')} style={{ background: '#00d38d', color: '#fff', border: 'none', borderRadius: 12, padding: '12px 28px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: "var(--font-cairo), 'Cairo', sans-serif" }}>
          العودة للرئيسية
        </button>
      </div>
    </div>
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box }
        ...
        .detail-item:hover { background: rgba(0,211,141,0.06) !important; border-color: rgba(0,211,141,0.22) !important; }
        input:focus { border-color: #00d38d !important; box-shadow: 0 0 0 3px rgba(0,211,141,0.14) !important; outline: none !important; }
        @media (max-width: 768px) {
      ...
      <div dir="rtl" style={{ minHeight: '100vh', background: '#f9fafb', color: '#0f172a' }}>
        ...
        <AnimatePresence>
          {toast ? (
            <motion.div
              ...
              className={`fixed bottom-4 left-1/2 z-[220] w-[calc(100%-2rem)] max-w-md -translate-x-1/2 rounded-lg border px-4 py-3 text-center text-sm font-bold shadow-sm backdrop-blur-sm ${
                toast.tone === 'ok'
        ...
        {/* ══ NAV ══ */}
        <nav style={{ position: 'sticky', top: 0, zIndex: 100, background: '#ffffff', borderBottom: '1px solid #e5e7eb', padding: '0 1.25rem', height: 48, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <a href="/" style={{ textDecoration: 'none', padding: '4px 0', borderRadius: 8 }}>
            <span style={{ fontSize: '1.25rem', fontWeight: 800, color: '#00d38d', fontFamily: "var(--font-cairo), 'Cairo', sans-serif" }}>دورلي</span>
          </a>
          <button onClick={() => router.back()} type="button" className="back-btn" style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '6px 16px', fontSize: 13, fontWeight: 700, color: '#64748b', cursor: 'pointer', fontFamily: "var(--font-cairo), 'Cairo', sans-serif", transition: 'background .2s' }}>
            ← رجوع
          </button>
        </nav>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2.5rem 1.5rem 6rem', position: 'relative', zIndex: 1 }}>
          ...
          {showAvailabilityBanner ? (
            <div
              className="fade-up mb-5 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-relaxed text-amber-900"
            >
              {availability === "rented"
          ...
          <div className="grid-layout" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '2rem', alignItems: 'start' }}>
            ...
            <div>
              ...
              {/* Hero image */}
              <div className="fade-up hero-img" style={{ borderRadius: 16, overflow: 'hidden', position: 'relative', height: 440, marginBottom: 12, background: '#f3f4f6', border: '1px solid #e5e7eb' }}>
                {hasImg ? (
                  <>
                    <Image
                    ...
                    />
                    <div aria-hidden style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(15,23,42,.35) 0%, transparent 50%)' }} />
                    {property.images.length > 1 && (
                      <div style={{ position: 'absolute', bottom: 16, left: 16, background: 'rgba(255,255,255,.92)', border: '1px solid #e5e7eb', borderRadius: 99, fontSize: 11, fontWeight: 700, color: '#64748b', padding: '4px 12px' }}>
              ...
              {embedUrl && (
                <div className="fade-up-2" style={{ marginBottom: 24 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                    ...
                    <span style={{ fontSize: 15, fontWeight: 800, color: '#0f172a' }}>جولة مرئية للعقار</span>
              ...
              <div className="fade-up-3" style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 16, padding: '1.5rem', marginBottom: '1.5rem' }}>
                <h2 style={{ fontSize: 15, fontWeight: 800, color: '#0f172a', marginBottom: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  {detailItems.map((item, i) => (
                    <div key={i} className="detail-item"
                      style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 12, padding: '14px 16px', transition: 'background .2s, border-color .2s', cursor: 'default' }}>
              ...
              {property.description && (
                <div className="fade-up-4" style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 16, padding: '1.5rem' }}>
                  <h3 style={{ fontSize: 15, fontWeight: 800, color: '#0f172a', marginBottom: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <span style={{ width: 26, height: 26, borderRadius: 8, background: `${meta.accent}20`, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 13 }}>📝</span>
                    وصف العقار
                  </h3>
                  <p style={{ fontSize: 14, color: '#64748b', lineHeight: 2, margin: 0 }}>{property.description}</p>
                </div>
              )}
            </div>
            ...
            <div className="sticky-col" style={{ position: 'sticky', top: 88 }}>
              ...
              {/* Price card */}
              <div className="fade-up-2" style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '1.6rem', marginBottom: '1rem' }}>
                {/* Type + rental badge */}
                ...
                {spamBlocked ? (
                  ...
                  <form onSubmit={handleLeadSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    <input
                      ...
                      style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '12px 14px', fontSize: 14, fontFamily: "var(--font-cairo), 'Cairo', sans-serif", color: '#0f172a', outline: 'none', transition: 'border-color .2s' }}
                    />
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                      <input
                        ...
                        style={{ background: '#ffffff', border: `1px solid ${phoneError ? '#ef4444' : '#e5e7eb'}`, borderRadius: 8, padding: '12px 14px', fontSize: 14, fontFamily: "var(--font-cairo), 'Cairo', sans-serif", color: '#0f172a', outline: 'none', transition: 'border-color .2s' }}
                      />
                      {phoneError && (
                    ...
                    <button type="submit" disabled={leadLoading} className="wa-btn"
                      style={{ marginTop: 4, background: '#25D366', color: '#fff', border: 'none', borderRadius: 12, padding: '14px', fontSize: 15, fontWeight: 700, cursor: leadLoading ? 'not-allowed' : 'pointer', fontFamily: "'Cairo', sans-serif", display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', opacity: leadLoading ? 0.7 : 1, transition: 'filter .2s, transform .2s' }}>
                      {leadLoading ? (
                  ...
                  <div style={{ background: 'rgba(0,211,141,.08)', border: '1px solid rgba(0,211,141,.25)', borderRadius: 16, padding: '1.5rem', textAlign: 'center' }}>
                    <div style={{ fontSize: 40, marginBottom: 10 }}>✅</div>
                    <p style={{ fontSize: 15, fontWeight: 700, color: '#00d38d', margin: '0 0 4px' }}>تم إرسال طلبك بنجاح!</p>
              ...
              <div className="fade-up-4" style={{ marginTop: '0.75rem', display: 'flex', gap: 8 }}>
                <button
                  ...
                  style={{ flex: 1, background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '11px', fontSize: 12, fontWeight: 700, color: '#64748b', cursor: 'pointer', fontFamily: "'Cairo', sans-serif", transition: 'background .2s, color .2s' }}
                  onMouseEnter={e => { e.currentTarget.style.color = '#475569'; e.currentTarget.style.background = '#f9fafb' }}
              ...
              {property.status === "active" ? (
                <button
                  ...
                  className="fade-up-4 mt-3 w-full rounded-lg border border-amber-200 bg-amber-50/80 px-3 py-2.5 text-center text-xs font-bold text-amber-800 transition hover:border-amber-300 hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-50"
          ...
          {reasonModalOpen && property ? (
            ...
            >
              <motion.div
                ...
                className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-lg shadow-slate-900/10"
              >
                <h3 className="mb-2 text-center text-base font-black text-slate-900">تفاصيل البلاغ</h3>
                <p className="mb-3 text-center text-xs text-slate-500">وصف مختصر يساعد فريق المراجعة (اختياري)</p>
                <textarea
                  value={reportReason}
                  onChange={(e) => setReportReason(e.target.value)}
                  rows={3}
                  className="mb-4 w-full resize-none rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-slate-800 outline-none ring-[#00d38d]/25 focus:ring-2"
                />
                <div className="flex flex-col gap-2 sm:flex-row-reverse">
                  <button
                    ...
                    className="flex-1 rounded-lg bg-[#00d38d] py-2.5 text-sm font-bold text-white shadow-sm disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 rounded-lg border border-gray-200 py-2.5 text-sm font-bold text-slate-600 hover:bg-gray-50"
                  >
                    إلغاء
                  </button>
                </div>
        ...
        {/* FOOTER */}
        <footer style={{ background: '#ffffff', borderTop: '1px solid #e5e7eb', padding: '2rem 1.5rem', textAlign: 'center' }}>
          <p style={{ fontSize: 18, fontWeight: 900, color: '#00d38d', marginBottom: '0.2rem', fontFamily: "var(--font-cairo), 'Cairo', sans-serif" }}>دورلي <span style={{ color: '#64748b', fontSize: 13, fontWeight: 600 }}>Dowarly</span></p>
